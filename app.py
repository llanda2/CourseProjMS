...
def process_shootings(df):
    ...
    # Create a cumulative year field
...
def render_incidents_map(tab, size_metric, color_metric, year_range):
    ...
    # Expand dataset to simulate cumulative effect
    expanded_rows = []
    for year in range(year_range[0], year_range[1] + 1):
        past_incidents = filtered_df[filtered_df['Year'] <= year].copy()
        past_incidents['Cumulative Year'] = year  # Important: mark the current frame
        expanded_rows.append(past_incidents)

    cumulative_df = pd.concat(expanded_rows, ignore_index=True)

    # Now create animated scatter mapbox
    fig = px.scatter_mapbox(
        cumulative_df,
        lat='latitude',
        lon='longitude',
        size=size_metric,
        color_discrete_sequence=["crimson"],  # <-- make all points crimson
        ...
        zoom=3,
        height=700,
        size_max=35,  # Bigger maximum circles
    )
